// <auto-generated />
using System;
using Kaloom.Students.Infrastructure.Data.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Kaloom.Students.Infrastructure.Migrations
{
    [DbContext(typeof(KaloomContext))]
    [Migration("20251127150916_UpdateStudentProperties")]
    partial class UpdateStudentProperties
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            MySqlModelBuilderExtensions.AutoIncrementColumns(modelBuilder);

            modelBuilder.Entity("AlunoInstituicao", b =>
                {
                    b.Property<int>("AlunosId")
                        .HasColumnType("int");

                    b.Property<int>("InstituicoesId")
                        .HasColumnType("int");

                    b.HasKey("AlunosId", "InstituicoesId");

                    b.HasIndex("InstituicoesId");

                    b.ToTable("AlunoInstituicoes", (string)null);
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.Aluno", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("DataCadastro")
                        .HasColumnType("datetime(6)");

                    b.Property<DateOnly>("DataNascimento")
                        .HasColumnType("date");

                    b.Property<int>("IdTipoAluno")
                        .HasColumnType("int");

                    b.Property<int>("IdUsuario")
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("varchar(16)");

                    b.Property<string>("Sobrenome")
                        .IsRequired()
                        .HasColumnType("varchar(24)");

                    b.HasKey("Id");

                    b.HasIndex("IdTipoAluno");

                    b.ToTable("Alunos");
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.Base.Instituicao", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("NomeUnidade")
                        .IsRequired()
                        .HasColumnType("varchar(60)");

                    b.HasKey("Id");

                    b.ToTable("Instituicao");

                    b.UseTptMappingStrategy();
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.TipoAluno", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("varchar(35)");

                    b.Property<bool>("Etec")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("Fatec")
                        .HasColumnType("tinyint(1)");

                    b.Property<int?>("StatusEtec")
                        .HasColumnType("int");

                    b.Property<int?>("StatusFatec")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("Fatec", "Etec", "StatusFatec", "StatusEtec")
                        .IsUnique();

                    b.ToTable("TipoAlunos", t =>
                        {
                            t.HasCheckConstraint("CK_StudentType_StatusEtec", "StatusEtec IN (1,2)");

                            t.HasCheckConstraint("CK_StudentType_StatusFatec", "StatusFatec IN (1,2)");
                        });

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Description = "Aluno Fatec",
                            Etec = false,
                            Fatec = true,
                            StatusFatec = 1
                        },
                        new
                        {
                            Id = 2,
                            Description = "Ex-Aluno Fatec",
                            Etec = false,
                            Fatec = true,
                            StatusFatec = 2
                        },
                        new
                        {
                            Id = 3,
                            Description = "Aluno Etec",
                            Etec = true,
                            Fatec = false,
                            StatusEtec = 1
                        },
                        new
                        {
                            Id = 4,
                            Description = "Ex-Aluno Etec",
                            Etec = true,
                            Fatec = false,
                            StatusEtec = 2
                        },
                        new
                        {
                            Id = 5,
                            Description = "Aluno Fatec e Etec",
                            Etec = true,
                            Fatec = true,
                            StatusEtec = 1,
                            StatusFatec = 1
                        },
                        new
                        {
                            Id = 6,
                            Description = "Aluno Fatec e Ex-Aluno Etec",
                            Etec = true,
                            Fatec = true,
                            StatusEtec = 2,
                            StatusFatec = 1
                        },
                        new
                        {
                            Id = 7,
                            Description = "Aluno Etec e Ex-Aluno Fatec",
                            Etec = true,
                            Fatec = true,
                            StatusEtec = 1,
                            StatusFatec = 2
                        },
                        new
                        {
                            Id = 8,
                            Description = "Ex-Aluno Fatec e Ex-Aluno Etec",
                            Etec = true,
                            Fatec = true,
                            StatusEtec = 2,
                            StatusFatec = 2
                        });
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.Etec", b =>
                {
                    b.HasBaseType("Kaloom.Students.Domain.Models.Base.Instituicao");

                    b.ToTable("Etecs", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            NomeUnidade = "Etec JK - Sede"
                        },
                        new
                        {
                            Id = 2,
                            NomeUnidade = "Etec JK - Extensão Céu Caminho do Mar"
                        },
                        new
                        {
                            Id = 3,
                            NomeUnidade = "Etec JK - Extensão Associação Comunitária Despertar"
                        },
                        new
                        {
                            Id = 4,
                            NomeUnidade = "Etec Lauro Gomes"
                        },
                        new
                        {
                            Id = 5,
                            NomeUnidade = "Etec Jorge Street"
                        });
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.Fatec", b =>
                {
                    b.HasBaseType("Kaloom.Students.Domain.Models.Base.Instituicao");

                    b.ToTable("Fatecs", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 6,
                            NomeUnidade = "Fatec Diadema - \"Luigi Papaiz\""
                        },
                        new
                        {
                            Id = 7,
                            NomeUnidade = "Fatec SBC - \"Adib Moisés Dib\""
                        },
                        new
                        {
                            Id = 8,
                            NomeUnidade = "Fatec São Paulo"
                        });
                });

            modelBuilder.Entity("AlunoInstituicao", b =>
                {
                    b.HasOne("Kaloom.Students.Domain.Models.Aluno", null)
                        .WithMany()
                        .HasForeignKey("AlunosId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Kaloom.Students.Domain.Models.Base.Instituicao", null)
                        .WithMany()
                        .HasForeignKey("InstituicoesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.Aluno", b =>
                {
                    b.HasOne("Kaloom.Students.Domain.Models.TipoAluno", "TipoAluno")
                        .WithMany("Alunos")
                        .HasForeignKey("IdTipoAluno")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("TipoAluno");
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.Etec", b =>
                {
                    b.HasOne("Kaloom.Students.Domain.Models.Base.Instituicao", null)
                        .WithOne()
                        .HasForeignKey("Kaloom.Students.Domain.Models.Etec", "Id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.Fatec", b =>
                {
                    b.HasOne("Kaloom.Students.Domain.Models.Base.Instituicao", null)
                        .WithOne()
                        .HasForeignKey("Kaloom.Students.Domain.Models.Fatec", "Id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Kaloom.Students.Domain.Models.TipoAluno", b =>
                {
                    b.Navigation("Alunos");
                });
#pragma warning restore 612, 618
        }
    }
}
